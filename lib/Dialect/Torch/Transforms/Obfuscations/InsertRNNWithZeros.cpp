...
static void insertRNNWithZeros(MLIRContext *context, Operation *f,
                               std::string activationFunc, int number, int layer) {
  ...
  if (opWorklist.empty() || layer > (int)opWorklist.size()) {
      llvm::errs() << "Not run InsertRNNWithZeros\n";
      return;
    }
  ...
  for (int i = 0; i < cycles; i++) {
    ...
    if (i == cycles - 1) { // 最后一次for循环对output进行修改
      ...
      slice = rewriter.create<AtenSliceTensorOp>(
          loc, rst.getType(), unsqueeze, int_dim, int_start,
          int_end, int1);
    }
  }
  ...
  Value add_rst = rewriter.create<AtenAddTensorOp>(
      loc, rst.getType(), rst, slice, float0);
  if (needReshape)
...
mlir::torch::Torch::createInsertRNNWithZerosPass(std::string activationFunc,
  ...
  return std::make_unique<InsertRNNWithZerosPass>(activationFunc, number, layer);
}
