...
static void insertRNN(MLIRContext *context, Operation *f, int number, int layer) {
  ...
  for (int i = 0; i < cycles; i++) {
    ...
    if (i == cycles - 1) { // 最后一次for循环对output进行修改
      ...
      slice = rewriter.create<AtenSliceTensorOp>(
          loc, rst.getType(), unsqueeze, int_dim,
          int_start, int_end, int1);
    }
  }
...
public:
  ...
  InsertRNNPass(int number, int layer) { this->number = number, this->layer = layer; }
